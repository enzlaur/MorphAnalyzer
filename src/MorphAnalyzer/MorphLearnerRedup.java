...
/*        for(int i=0;i<v.size();i++) {
    ...
    /**
     * Main method for finding the root of the word (I think).
    ...
    public MAResult analyzeMultipleMod(String orig) 
    {
        ...
        // Show orig's previous content
        println("orig before reduceInFix: " + orig);
        
        infixes = reduceInfix(orig);
        
        //show orig's content after performing reduceInFix
        println("orig after reduceInfix: " + infixes);        
        
        //prints the number of infixes found
        for(l=0;l<infixes.size();l++) 
        {
        	println("l<infixes where l = " + infixes.capacity());
            reducedWord = infixes.elementAt(l).infixRemove(orig);
            //print reducedWord
            println("reducedWord: " + reducedWord);            
            ...
            for(j=0;j<suffixes.size();j++) 
            {
            	println("j<suffixes where j = " + j);
            	
                prefix="";
                println("Removing : " + suffixes.elementAt(j));
                suffix = suffixes.elementAt(j);
                tempResult = rewriteMultipleNoSemantic(reducedWord,prefix,suffix);
           ...
           // no suffix
            for(i=0;i<prefixes.size();i++)
            {
            	println("i<prefixes where i = " + i);
            	
                prefix = prefixes.elementAt(i);
                println(prefixes.elementAt(i));
                suffix = "";
                tempResult = rewriteMultipleNoSemantic(reducedWord,prefix,suffix);
            ...
            // moved suffix after prefix removal
            for(j=0;j<suffixes.size();j++) 
            {
            	println("j<suffixes where j = " + j);
            	
                prefix="";
                // Removing the selected suffix in the word
                println("Removing : " + suffixes.elementAt(j));
                suffix = suffixes.elementAt(j);                
                tempResult = rewriteMultipleNoSemantic(reducedWord,prefix,suffix);
